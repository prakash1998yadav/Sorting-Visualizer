...
function generateBars(n = -1) {
	...
	container.style.flexDirection = 'rowp';
...
let initialBarsState = [];

    function resetBars() {
      let container = document.getElementById("container");
      container.innerHTML = initialBarsState.join('');
      isPaused = false;
      currentStep = 0;
    }
...
async function SelectionSort() {
    ...
    for (let i = 0; i < bars.length; i++) {
        ...
        await SleepWithPause(delay / 5.0);
        ...
        await SleepWithPause(2 * delay / 5.0);
        ...
        steps.push(() => {
            // This function represents the state of bars at this step
            container.innerHTML = bars.join('');
            document.getElementById(curr_id).style.backgroundColor = def;
            document.getElementById(nxt_ele).style.backgroundColor = def;
        });
